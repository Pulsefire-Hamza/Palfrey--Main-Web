        ...
        <div className="block sm:hidden p-4">
          <div className="flex gap-3">
              ...
              <div className="text-xs text-gray-500 mt-1 space-y-1">
                ...
                {item.measurements && (
                  <div className="text-xs space-y-0.5 mt-2">
                    {item.measurements.length && <div>Length: {item.measurements.length}</div>}
                    ...
                    {item.measurements.hip && <div>Hip: {item.measurements.hip}</div>}
                    {item.measurements.sleeve && <div>Sleeve: {item.measurements.sleeve}</div>}
                    {item.measurements.armhole && <div>Armhole: {item.measurements.armhole}</div>}
                    {item.measurements.neckDepthFront && <div>Neck Front: {item.measurements.neckDepthFront}</div>}
                    {item.measurements.neckDepthBack && <div>Neck Back: {item.measurements.neckDepthBack}</div>}
                  </div>
                )}
                {item.id === 8 && (
                  <>
                    {item.scent && (
                      <span>
                        {item.size ? " | " : ""}Scent: {item.scent}
                      </span>
                    )}
                    {item.kilos && <span> | Kilos: {item.kilos}kg</span>}
                  </>
                )}
              </div>
            ...
            <button
              ...
              onClick={() => removeFromCart(item.id, item.withDupatta, item.size)}
          ...
          <div className="flex justify-between items-center mt-3">
            <div className="flex items-center border rounded-lg">
              <button
                ...
                onClick={() => updateQuantity(item.id, item.withDupatta, item.size, item.quantity - 1)}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="text-center bg-white p-8 sm:p-12 rounded-lg shadow-sm max-w-md w-full">
          ...
          <a
            href="/"
            className="inline-block bg-black text-white px-6 py-3 rounded-lg hover:bg-gray-900 transition-colors w-full sm:w-auto"
          >
            Start Shopping
          </a>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          <div className="divide-y divide-gray-100">
            {cartItems.map((item, index) => (
              <CartRow key={`${item.id}-${item.size}-${item.withDupatta}-${index}`} item={item} />
            ))}
          </div>
          ...
          <div className="border-t p-4 sm:p-6">
            ...
            <textarea
              className="w-full p-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent resize-none text-sm"
              rows={3}
          ...
          <div className="border-t p-4 sm:p-6 bg-gray-50">
            ...
            <div className="space-y-3 sm:space-y-0 sm:flex sm:gap-3">
              ...
              <button
                className="block w-full sm:flex-1 bg-black text-white py-3 px-6 rounded-lg hover:bg-gray-900 transition-colors font-medium"
